...
export default _make('Pubsub', [
    ...
    * _normalizeUntilRejectConfig (rejectConfig, stageName) {
        switch (typeof rejectConfig) {
            case 'object':
                if (!rejectConfig[Symbol.iterator]) {
                    rejectConfig = [
                        rejectConfig
                    ];
                }
        ...
        for (const rejectConfigItem of rejectConfig) {
            switch (typeof rejectConfigItem) {
                case 'string':
                case 'symbol':
                    yield {
                        eventName: rejectConfigItem,
                        stageName
                    };
    ...
    _createUntilMethod ({
        ...
        return function (config) {
            ...
            try {
                subscriptions.push(this[bulkSubscribeMethodName]({
                    ...subscriptionConfig,
                ...
                for (const rejectConfigItem of this._normalizeUntilRejectConfig(rejectConfig, stageName)) {
                    if (cancel.completed) {
                        break;
                    }
            ...
            return Object.defineProperties(promise, {
                ...
                subscribed: {
                    get () {
                        return subscriptions.some(subscription => subscription.subscribed);
                    }
                },
                unsubscribe: {
                    value () {
